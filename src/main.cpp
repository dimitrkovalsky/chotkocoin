...
#if defined(NDEBUG)
# error "Chotkocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x88474cbc76302d26c3932d31e788a35e6304d162529f00402efa12be24c8c23b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chotkocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Chotkocoin Signed Message:\n";
...
{
    // Chotkocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chotkocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chotkocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9eb22ad43a6b65f22a6d7a6f7fd48a696f2725d428baf90d064af15f8a9d2001");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Chotko coin was created";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0433745b1fb3162526929cdf05e2fa4511ff4f7b41d37eab3cfda07c5f45380a33f699e10ac36a9be68ed7763d0c263c056717ceebdb323c2cdb17e478bd4553f4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515930020;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084691823;

        if (fTestNet)
        {
            block.nTime    = 1515929020;
            block.nNonce   = 385351902;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd4eaf8d9cb0dec3b532bf95c0a7b79be38c9633d56fe2c26df848d07067d39e3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Chotkocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChotkocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChotkocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChotkocoinMiner(CWallet *pwallet)
{
    printf("ChotkocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chotkocoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChotkocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChotkocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChotkocoinMiner, pwallet));
}
